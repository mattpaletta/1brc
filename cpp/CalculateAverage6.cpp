...
public:
    ...
    Measurement* Get(const std::string& name) {
        ...
        std::int64_t l = 0;
        std::int64_t r = this->_size - 1;

        while (l <= r) {
            std::int64_t m = l + (r - l) / 2;
            if (static_cast<size_t>(m) >= this->_size) {
                return nullptr;
            }

            // name < data[mid]
            if (name == this->_data[m].first) {
                return &this->_data[m].second;
            }
            
            if (this->locale(this->_data[m].first, name)) {
                l = m + 1;
